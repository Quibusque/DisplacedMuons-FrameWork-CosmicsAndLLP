...
_vars = {
    "dmu_dsa_dxy": {"name": r"tag |d_0| (cm)", "nbins": 20, "xmin": 0.0, "xmax": 200.0},
    "dmu_dsa_pt": {"name": r"tag p_T (GeV)", "nbins": 30, "xmin": 0.0, "xmax": 90.0},
    "dmu_dsa_eta": {"name": r"tag #eta", "nbins": 20, "xmin": -1.0, "xmax": 1.0},
    "dmu_dgl_dxy": {
        "name": r"tag |d0| (cm)",
        "nbins": 6,
        "xbins": np.array([0.0, 2.0, 5.0, 10.0, 30.0, 50.0, 70.0], dtype=float),
    },
    "dmu_dgl_dz": {
        "name": r"tag |dz| (cm)",
        "nbins": 6,
        "xbins": np.array([0.0, 8.0, 20.0, 40.0, 60.0, 90.0, 140.0], dtype=float),
    },
}

parser = ArgumentParser()
parser.add_argument(
    "--var", type=str, nargs="*", choices=_vars.keys(), help="Variable(s) to plot"
)
parser.add_argument(
    "--mcfile",
    type=str,
    help="MC Ntuple file",
    default="CosmicsMC_Leonardo_Ntuples.root",
)
parser.add_argument(
    "--datafile", type=str, help="Data Ntuple file", default="CosmicsData_Ntuples.root"
)
...
if __name__ == "__main__":
    ...
    if len(args.var) == 1:
        ...
        xmax = _vars[var]["xmax"]
        h_dummy = R.TH1F(
            "h_dummy", ";" + _vars[var]["name"] + ";Efficiency", nbins, xmin, xmax
        )
        h_data_total = R.TH1F(
            "h_data_total", ";" + _vars[var]["name"] + ";Efficiency", nbins, xmin, xmax
        )
        h_data_pass = R.TH1F(
            "h_data_pass", ";" + _vars[var]["name"] + ";Efficiency", nbins, xmin, xmax
        )
        h_mc_total = R.TH1F(
            "h_mc_total", ";" + _vars[var]["name"] + ";Efficiency", nbins, xmin, xmax
        )
        h_mc_pass = R.TH1F(
            "h_mc_pass", ";" + _vars[var]["name"] + ";Efficiency", nbins, xmin, xmax
        )
        h_data_total.Sumw2()
        h_data_pass.Sumw2()
        h_mc_total.Sumw2()
        ...
        aux_frame2.DrawLine(
            pad2.GetUxmax(), pad2.GetUymin(), pad2.GetUxmax(), pad2.GetUymax()
        )
        ...
        h_dummy = R.TH2F(
            "h_dummy",
            r";" + _vars[varx]["name"] + ";" + _vars[vary]["name"],
            nbinsx,
            xbinsx,
            nbinsy,
            xbinsy,
        ...
        h_data_total = R.TH2F(
            "h_data_total",
            r"Cosmics Data Run2023C;" + _vars[varx]["name"] + ";" + _vars[vary]["name"],
            nbinsx,
            xbinsx,
            nbinsy,
            xbinsy,
        ...
        h_mc_total = R.TH2F(
            "h_mc_total",
            r"Cosmics MC;" + _vars[varx]["name"] + ";" + _vars[vary]["name"],
            nbinsx,
            xbinsx,
            nbinsy,
            xbinsy,
        ...
        latex.DrawLatex(
            ...
            + f" (Cosmics {datafilename.split('_')[1].split('-')[1]})",
        )
